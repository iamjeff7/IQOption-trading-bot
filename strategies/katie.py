...
def get_action(d):
    ...
    #print(f'BBANDS: {upper[-1]} {lower[-1]}')
    #print(f'   CCI: {real[-1]}')
    #print(f' STOCH: {slowk[-1]} {slowd[-1]}')
    #print(close[-1] > upper[-1], real[-1] > 100, slowk[-1] > 80, slowd[-1] > 80)
    #print(close[-1] < lower[-1], real[-1] < -100, slowk[-1] < 20, slowd[-1] < 20)
    if close[-1] > upper[-1] and real[-1] > 100 and slowk[-1] > 80 and slowd[-1] > 80:
        #return 'call'
        return 'put'
    elif close[-1] < lower[-1] and real[-1] < -100 and slowk[-1] < 20 and slowd[-1] < 20:
        #return 'put'
        return 'call'
    else:
        return None
